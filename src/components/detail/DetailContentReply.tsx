    ...
    <Box>
      ...
      <Box sx={{ padding: 2, display: "flex", gap: 2, borderBottom: "1px solid gray" }}>
        <Box sx={{ display: "flex", alignItems: "start" }}>
          <img style={{ width: "40px", height: "40px", objectFit: "cover", borderRadius: 50 }} src={`${baseUrl.baseUrlImg}${post?.author?.profile_pic}`} alt="" />
        </Box>
        <Box>
            ...
            <ImageList sx={{ width: "100%" }}>
              {post?.images.map((item) => (
                <ImageListItem key={item.image}>
                  <img src={`${baseUrl.baseUrlImg}${item.image}`} srcSet={`${baseUrl.baseUrlImg}${item.image}`} alt={item.image} loading="lazy" />
                </ImageListItem>
              ))}
            </ImageList>
          )}
